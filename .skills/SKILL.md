...
- The reader family: 12, 14, 16, 18pt (4 styles each: regular, bold, italic, bold-italic)
- Ubuntu UI fonts: 10, 12pt (2 styles)
- Noto Sans 8pt regular (backs `SMALL_FONT_ID` only)

Each reader family costs ~1.08 MB of the 6.25 MB OTA slot, so a build ships **exactly one** and
relies on SD card fonts for everything else. Which one is a build-time choice:

| Env | Built-in reader family |
|---|---|
| `default` | Bitter (slab serif drawn for e-ink) |
| `notoserif` | Noto Serif (`-DREADER_FONT_NOTOSERIF`) |

Code never names the family directly — `src/readerFont.h` exposes `READER_FONT_DATA(size, style)`,
`READER_FONT_{12,14,16,18}_ID` and `READER_FONT_NAME_STR_ID`, and `builtinFonts/all.h` includes
only the selected family's headers. Adding a variant means: headers in `builtinFonts/`, a branch in
`all.h` and `readerFont.h`, an env in `platformio.ini`, and the env name in `DEV_ENVS`
(`scripts/git_branch.py`) so `CROSSPOINT_VERSION` is injected.

Font IDs are regenerated by `lib/EpdFont/scripts/build-font-ids.sh`; changing one invalidates
section caches, which record `spec.fontId`.

**Total**: ~25 global `EpdFont` and `EpdFontFamily` objects
