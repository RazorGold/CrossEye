...
class BookStatsActivity final : public Activity {
 ...
 public:
  ...
  // CrossInk lets the Power button double as Confirm on reader-mode screens.
  // CrossPoint has no such concept, so the override is dropped rather than
  // importing a base-class virtual that nothing here consults. Confirm still works.
};
