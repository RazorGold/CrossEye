...
void BookStatsActivity::exitStatsActivity(const bool viaBack) {
  // CrossInk suppresses the triggering button's release here so the parent activity
  // does not re-handle the same physical release. CrossPoint's MappedInputManager has
  // no suppression mechanism and its other activities exit without one, so the calls
  // are dropped rather than adding state to shared input handling. If leaving the
  // stats screen ever skips a level (Back exiting two activities at once), porting
  // suppressNextBackRelease/suppressNextConfirmRelease is the fix.
  (void)viaBack;
